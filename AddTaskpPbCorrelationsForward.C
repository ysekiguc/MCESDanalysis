								       ...
								       )
{
  ...
  myTask->SetAssociatedTrack(kFALSE);
  //	myTask->SetPID(fpid);
  ...
  /*
  AliAnalysisTask* fwd = mgr->GetTask("Forward");  
  AliAnalysisDataContainer* exc = mgr->CreateContainer("FWD",AliAODForwardMult::Class(),  AliAnalsyisManager::kExchangeContainer, 0);
  mgr->ConnectOutput(fwd,4,exc);
  mgr->ConnectInput(myTask,5,exc);
  */
